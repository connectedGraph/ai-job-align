...
在传统的招聘和求职场景中，人岗匹配往往面临三类痛点：
1. **非标词泛滥（Synonym Noise）**：例如“React.js”、“react”、“前端开发框架”等在原始数据中格式混乱，直接进行字符匹配极易漏诊。
2. **整段向量匹配黑箱（Opaque Embedding Match）**：将学生简历或合成画像文本与岗位 JD 直接做 embedding 相似度匹配，往往只能得到一个看似精确的 cosine 分数，却难以解释“具体命中了哪些能力、缺失哪些技能、能力等级是否达标”。文本相似并不等同于真实能力匹配，尤其在岗位要求涉及**技术深度（Level）**、**基础原理（Principle）**、**工程经验（Engineering）**以及**软素质与潜力**时，黑箱相似度容易产生误判。
3. **求职者缺少可理解的能力画像图谱（Profile Graph）**：传统推荐系统只给岗位结果，不帮助学生理解自己的能力结构。本系统将学生能力拆解为技术栈、技术能力、开发工具、软素质、成长潜力等可计算维度，并通过等级、缺口和证据链呈现，让“构建画像”的过程本身成为一次自我认知和职业规划过程。

本系统的核心思路不是“学生整体向量 vs 岗位整体向量”的一次性匹配，而是先构建可解释的学生/岗位能力画像图谱，再用 embedding 做语义召回、标准词对齐和相似标签补齐，最终通过**逐标签、分维度、带等级差的确定性打分算法（Deterministic Alignment）**完成人岗匹配，输出可追溯、可解释、可行动的推荐结果。
